...
/// attached to. `whelk > log` redirects stdout and stays interactive; nothing a
...
///
/// The fix is to ignore `SIGTTOU` across the call. `whelk` ignores it for the
...
mod tests {
    ...
    fn a_test_harness_has_no_controlling_terminal() {
        ...
        // tests drive `whelk` through a pipe and check that it *declines* to do
