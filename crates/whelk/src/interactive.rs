...
use whelk_executor::Shell;
use whelk_line::{Action, Decoded, Editor, History};
...
const HISTORY_FILE: &str = ".whelk_history";
...
impl Interactive {
    ...
    fn signals(&mut self, prompt: &str, shell: &mut Shell) -> std::io::Result<Option<Input>> {
        ...
        if shell.take_resize() {
            ...
            write!(out, "{}", whelk_line::render_line(prompt, &self.editor))?;
            out.flush()?;
        }
    ...
    pub fn read_line(&mut self, prompt: &str, shell: &mut Shell) -> std::io::Result<Input> {
        ...
        let _raw = match whelk_terminal::raw() {
            Ok(guard) => guard,
        ...
        loop {
            let ready = match self.poller.wait(None) {
                ...
                Err(whelk_event::INTERRUPTED) => continue,
                Err(error) => return Err(std::io::Error::from_raw_os_error(error as i32)),
            };
            ...
            while let Decoded::Key(key, used) = whelk_line::decode(&pending) {
                pending.drain(..used);
                ...
                match self.editor.handle(key, &self.completer) {
                    ...
                    Action::Suggest(candidates) => {
                        write!(
                            out,
                            "{}",
                            whelk_line::render_suggestions(prompt, &self.editor, &candidates)
                        )?;
                    }

                    Action::Clear => {
                        write!(out, "{}", whelk_line::CLEAR_SCREEN)?;
                        write!(out, "{}", whelk_line::render_line(prompt, &self.editor))?;
                    }
