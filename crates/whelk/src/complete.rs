...
//!
//! This lives in the binary rather than in `whelk-line`, because completing a
...
//! microseconds. `whelk-line` asks through a trait; this answers.
//!
...
use whelk_line::{Completer, Completion};
...
mod tests {
    ...
    impl Scratch {
        fn new(tag: &str) -> Self {
            let dir =
                std::env::temp_dir().join(format!("whelk-complete-{}-{tag}", std::process::id()));
    ...
    fn a_dollar_completes_environment_variables() {
        std::env::set_var("WHELK_COMPLETION_TEST", "1");
        let found = Shell.complete("echo $WHELK_COMPLETION", 20);
        assert_eq!(found.start, 5);
        assert!(
            found
                .candidates
                .contains(&"$WHELK_COMPLETION_TEST".to_owned()),
            "{:?}",
            found.candidates
        );
    }
