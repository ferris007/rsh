//! `whelk` — a Unix shell built from first principles.
//!
...
//! table lives in `whelk-process`; everything that decides what a line means
//! lives in `whelk-parser` and `whelk-executor`.
...
use whelk_executor::{Outcome, Shell};
...
usage:
  whelk                 start a shell; a terminal gets editing, history, and completion
  whelk < script        read commands from a file or a pipe, without prompts
  whelk --benchmark     measure startup, a command, and a pipeline
  whelk --help          print this
  whelk --version       print the version
...
enum Request {
    ...
    /// A bare word, which is almost always someone expecting `whelk script.sh`.
...
fn main() -> ExitCode {
    ...
    match request(std::env::args().skip(1)) {
        ...
        Request::Version => {
            println!("whelk {}", env!("CARGO_PKG_VERSION"));
            return ExitCode::SUCCESS;
        }
        ...
        Request::Benchmark => {
            return match benchmark::run() {
                Ok(()) => ExitCode::SUCCESS,
                Err(error) => {
                    eprintln!("whelk: benchmark failed: {error}");
                    ExitCode::from(1)
                }
            };
        }
        ...
        Request::UnknownOption(option) => {
            eprintln!("whelk: unknown option `{option}`");
            eprintln!("      try `whelk --help`");
            return ExitCode::from(EXIT_USAGE as u8);
        }

        Request::NotAScriptRunner(name) => {
            eprintln!("whelk: commands are read from standard input, not from a file by name");
            eprintln!("      try `whelk < {name}`");
            return ExitCode::from(EXIT_USAGE as u8);
        }
    }
    ...
    if let Err(error) = shell.install_signal_handlers() {
        eprintln!("whelk: cannot install signal handlers: {error}");
    ...
    let status = loop {
        ...
        match read {
            ...
            Err(error) => {
                eprintln!("whelk: failed to read input: {error}");
                break 1;
            }
        }
    };
