...
fn run(script: &str) -> (String, String, i32) {
    ...
    (
        ...
        out.status.code().expect("whelk was killed by a signal"),
    )
}
...
fn signal(child: &Child, signal: Signal) {
    kill(Pid::from_raw(child.id() as i32), signal).expect("failed to signal whelk");
}
...
fn exit_status_after(sig: Signal) -> std::process::ExitStatus {
    ...
    let status = child.wait().expect("failed to wait for whelk");
    drop(stdin);
    status
}
