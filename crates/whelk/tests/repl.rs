...
fn run_full(script: &str, vars: &[(&str, &str)], dir: &Path) -> Session {
    let mut command = Command::new(WHELK);
    ...
    child
        ...
        .expect("failed to write script to whelk");

    Session(
        child
            .wait_with_output()
            .expect("failed to collect whelk output"),
    )
}
...
impl Session {
    ...
    fn code(&self) -> i32 {
        self.0.status.code().expect("whelk was killed by a signal")
    }
}
...
impl Scratch {
    fn new() -> Self {
        ...
        let dir = std::env::temp_dir().join(format!("whelk-repl-{}-{n}", std::process::id()));
...
fn children_inherit_the_default_action_for_sigpipe() {
    ...
    // exec. Without an explicit reset in the child, every program `whelk` runs
...
fn no_prompt_when_input_is_not_a_terminal() {
    ...
    // silent about it.
    let session = run("echo hello\n");
    assert!(!session.stdout().contains("whelk>"));
    assert!(!session.stderr().contains("whelk>"));
}
