...
//!
//! Off unless `RSH_TRACE` is set, and close to free when it is off: one relaxed
...
///
/// Read from `RSH_TRACE` the first time it is asked, and cached — a shell that
/// consulted the environment on every parse would be measuring the environment.
pub fn enabled() -> bool {
    *ENABLED.get_or_init(|| {
        std::env::var_os("RSH_TRACE").is_some_and(|value| value != "0" && !value.is_empty())
    })
}
