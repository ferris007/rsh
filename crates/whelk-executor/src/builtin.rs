...
use whelk_job::{JobSpec, JobState, JobTable};
...
fn cd(args: &[String]) -> i32 {
    if args.len() > 1 {
        eprintln!("whelk: cd: too many arguments");
        return EXIT_USAGE;
    }
    ...
    let target: PathBuf = match args.first().map(String::as_str) {
        None | Some("~") => match std::env::var_os("HOME") {
            ...
            None => {
                eprintln!("whelk: cd: HOME not set");
                return EXIT_USAGE;
            }
        },
        Some("-") => match std::env::var_os("OLDPWD") {
            ...
            None => {
                eprintln!("whelk: cd: OLDPWD not set");
                return EXIT_USAGE;
            }
        },
    ...
    if let Err(error) = std::env::set_current_dir(&target) {
        eprintln!("whelk: cd: {}: {error}", target.display());
        return EXIT_USAGE;
    }
    ...
    match args {
        ...
        [code] => match code.parse::<i32>() {
            ...
            Err(_) => {
                ...
                eprintln!("whelk: exit: {code}: numeric argument required");
        ...
        _ => {
            ...
            // asking again.
            eprintln!("whelk: exit: too many arguments");
...
fn fg(args: &[String], ctx: &mut Context<'_>) -> i32 {
    if !ctx.job_control {
        eprintln!("whelk: fg: no job control without a terminal");
        return EXIT_USAGE;
    }
    ...
    let resumed_modes = whelk_terminal::snapshot();
    let _ = whelk_terminal::give_to(ctx.shell_pgid);
...
fn bg(args: &[String], ctx: &mut Context<'_>) -> i32 {
    if !ctx.job_control {
        eprintln!("whelk: bg: no job control without a terminal");
        return EXIT_USAGE;
    }
...
fn select(
    ...
    ctx: &Context<'_>,
    builtin: &str,
) -> Option<(whelk_job::JobId, nix::unistd::Pid, String)> {
    if args.len() > 1 {
        eprintln!("whelk: {builtin}: too many arguments");
        return None;
    }
    ...
    match ctx.jobs.find(spec) {
        ...
        Some(job) => {
            eprintln!("whelk: {builtin}: job {} has finished", job.id());
            None
        }
        None => {
            eprintln!("whelk: {builtin}: no such job");
            None
        }
    }
}
...
fn wait_for(ctx: &mut Context<'_>, id: whelk_job::JobId, command: &str) -> i32 {
    loop {
        let events = whelk_process::collect_child_events_blocking();
