...
use whelk_job::JobTable;
use whelk_parser::Pipeline;
use whelk_process::{Child, ExitStatus, Pipe, Redirections, Waited};
...
pub enum PipelineError {
    ...
    Process(whelk_process::SpawnError),
...
pub struct Context<'a> {
    ...
    /// blind.
    pub shell_modes: Option<&'a whelk_terminal::Modes>,
}
...
pub fn run(
    ...
    for (index, command) in commands.iter().enumerate() {
        ...
        let argv = {
            let _span = whelk_trace::span!("expand", words = command.words().len());
    ...
    let spawning = whelk_trace::span!("spawn", stages = commands.len());
...
fn foreground(children: Vec<Spawned>, pgid: Pid, ctx: &mut Context<'_>, command_text: &str) -> i32 {
    if ctx.job_control {
        ...
        let _ = whelk_terminal::give_to(pgid);
    }

    let _span = whelk_trace::span!("wait");
        ...
        match spawned {
            ...
            Spawned::Running(child) => {
                ...
                match await_stage(&child, ctx.job_control, command_text) {
                    ...
                    Err(error) => {
                        eprintln!("whelk: {error}");
                        status = whelk_process::EXIT_NOT_EXECUTABLE;
                    }
                }
            }
        }
    ...
    if ctx.job_control {
        ...
        let _ = whelk_terminal::give_to(ctx.shell_pgid);

        if let Some(modes) = ctx.shell_modes {
            let _ = whelk_terminal::restore(modes);
        }
    }
    ...
    if stopped {
        ...
        if let Some(job) = ctx.jobs.find_mut(whelk_job::JobSpec::Id(id)) {
            job.stopped();
            job.mark_reported();
...
fn await_stage(
    ...
    job_control: bool,
    command_text: &str,
) -> Result<Option<ExitStatus>, whelk_process::SpawnError> {
    loop {
        match child.wait_or_stop()? {
            ...
            Waited::Stopped(signal) => {
                eprintln!(
                    "whelk: {command_text}: stopped by {signal}; continuing (no terminal for job control)"
                );
                child.resume()?;
            }
        }
...
fn spawn(stage: Stage, pgid: Option<Pid>, job_control: bool) -> Spawned {
    ...
    let resolved = {
        let _span = whelk_trace::span!("resolve", program = program);
        whelk_process::resolve(program, std::env::var_os("PATH").as_deref())
    };
    ...
    let path = match resolved {
        ...
        Err(error) => {
            eprintln!("whelk: {error}");
            if let whelk_process::ResolveError::NotFound { .. } = &error {
                if let Some(closest) =
                    whelk_process::suggest(program, std::env::var_os("PATH").as_deref())
                {
                    eprintln!("      did you mean `{closest}`?");
                }
            }
            return Spawned::Failed(match error {
                whelk_process::ResolveError::NotFound { .. } => whelk_process::EXIT_NOT_FOUND,
                whelk_process::ResolveError::NotExecutable { .. } => {
                    whelk_process::EXIT_NOT_EXECUTABLE
                }
            });
        }
    };

    let prepared = match whelk_process::Command::new(&path, &stage.argv) {
        Ok(prepared) => {
        ...
        Err(error) => {
            eprintln!("whelk: {error}");
            return Spawned::Failed(whelk_process::EXIT_NOT_EXECUTABLE);
        }
    };
    ...
    match prepared.spawn() {
        ...
        Err(error) => {
            eprintln!("whelk: {program}: {error}");
            Spawned::Failed(whelk_process::EXIT_NOT_EXECUTABLE)
        }
    }
}
