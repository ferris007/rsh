...
use nix::errno::Errno;
use whelk_parser::{Command, RedirectKind, Span};
use whelk_process::Redirections;
...
impl fmt::Display for RedirectError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            // matching that keeps `whelk`'s messages greppable alongside theirs.
...
mod tests {
    ...
    impl Scratch {
        fn new() -> Self {
            ...
            let dir =
                std::env::temp_dir().join(format!("whelk-redirect-{}-{n}", std::process::id()));
