...
use whelk_parser::{Command, ParseError, Pipeline, Span};
use whelk_process::{Redirections, ResolveError, Restore, EXIT_NOT_EXECUTABLE, EXIT_NOT_FOUND};
...
pub struct Shell {
    ...
    terminal_modes: Option<whelk_terminal::Modes>,
...
impl Default for Shell {
    fn default() -> Self {
        Self {
            last_status: 0,
            jobs: JobTable::new(),
            job_control: whelk_terminal::is_interactive(),
            shell_pgid: getpgrp(),
            warned_about_jobs: false,
            terminal_modes: whelk_terminal::snapshot(),
...
impl Shell {
    ...
    pub fn refresh_window_size(&mut self, resized: bool) {
        if !self.window_size_known || resized {
            if let Some(size) = whelk_terminal::size() {
    ...
    pub fn report_jobs(&mut self) {
        if !whelk_process::take_child_event() && self.jobs.is_empty() {
            return;
        }

        let events = whelk_process::collect_child_events();
    ...
    pub fn install_signal_handlers(&self) -> Result<(), Errno> {
        whelk_process::install_signal_handlers()
    }

    /// Whether a Ctrl-C has arrived since this was last called.
    pub fn take_interrupt(&self) -> bool {
        whelk_process::take_interrupt()
    }
    ...
    pub fn shutdown_requested(&self) -> Option<i32> {
        whelk_process::shutdown_requested().map(|signal| signal as i32)
    }
    ...
    pub fn confirm_exit(&mut self) -> bool {
        let stopped = self
            .jobs
            .iter()
            .any(|job| matches!(job.state(), whelk_job::JobState::Stopped));

        if stopped && !self.warned_about_jobs {
            eprintln!("whelk: there are stopped jobs");
    ...
    fn dispatch(&mut self, line: &str) -> Outcome {
        let parsed = {
            let _span = whelk_trace::span!("parse", input = line.len());
            whelk_parser::parse(line)
        };
...
fn run_external(argv: &[String], redirections: Redirections) -> i32 {
    ...
    let path = match whelk_process::resolve(program, std::env::var_os("PATH").as_deref()) {
        Ok(path) => path,
        Err(error) => {
            eprintln!("whelk: {error}");
            return match error {
    ...
    let prepared = match whelk_process::Command::new(&path, argv).map(|command| {
    ...
    let waited = prepared.spawn().and_then(|child| {
        child.wait_with(|signal| {
            ...
            // keystroke.
            eprintln!("whelk: {program}: stopped by {signal}; continuing (job control is phase 6)");
        })
    });
