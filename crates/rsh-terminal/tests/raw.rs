...
fn raw_mode_changes_what_the_driver_does_and_puts_it_back() {
    ...
    let raw = with_a_terminal(|| {
        let _guard = rsh_terminal::raw().expect("failed to enter raw mode");
        describe_flags();
    });
    assert!(
    ...
    let by_hand = with_a_terminal(|| {
        let saved = rsh_terminal::snapshot().expect("no terminal");
        std::mem::forget(rsh_terminal::raw().expect("failed to enter raw mode"));
        rsh_terminal::restore(&saved).expect("failed to restore");
        describe_flags();
    });
    assert!(
