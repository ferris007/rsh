...
//!
//! The editor's own logic is unit-tested in `rsh-line`, where a keystroke is a
...
impl Home {
    fn new(tag: &str) -> Self {
        let dir = std::env::temp_dir().join(format!("rsh-home-{}-{tag}", std::process::id()));
...
fn up_recalls_the_previous_command() {
    ...
    session.type_in(UP);
    session.expect("rsh> echo remembered");
}
...
fn history_survives_between_sessions() {
    ...
    assert!(
        home.read(".rsh_history").contains("echo persisted"),
        "history was not written: {:?}",
        home.read(".rsh_history")
    );
    ...
    second.type_in(UP);
    second.expect("rsh> exit");
    second.type_in(UP);
    second.expect("rsh> echo persisted");
}
...
fn the_configuration_file_runs_before_the_first_prompt() {
    ...
    home.write(".rshrc", "echo from the config file\n");
...
fn a_bad_line_in_the_configuration_does_not_stop_the_rest() {
    ...
    home.write(".rshrc", "definitely-not-a-command\necho still started\n");
