...
mod tests {
    ...
    fn a_redraw_returns_erases_and_rewrites() {
        let drawn = line("whelk> ", &editor_with("echo hi", 0));
        assert!(
            drawn.starts_with("\r\x1b[K"),
            "should return and erase: {drawn:?}"
        );
        assert!(drawn.contains("whelk> echo hi"));
    }

    #[test]
    fn the_cursor_ends_up_after_the_prompt_and_the_text() {
        // 7 for "whelk> " plus 7 for "echo hi".
        let drawn = line("whelk> ", &editor_with("echo hi", 0));
        assert!(
            drawn.ends_with("\r\x1b[14C"),
    ...
    fn a_cursor_moved_left_lands_earlier() {
        // Two characters left of the end: 14 - 2.
        let drawn = line("whelk> ", &editor_with("echo hi", 2));
        assert!(
