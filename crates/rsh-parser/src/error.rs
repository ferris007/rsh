...
pub enum ParseError {
    ...
    ///
    /// A real shell would keep reading with a continuation prompt. `rsh` has no
    ...
    /// something real; it is `rsh` that is incomplete.
    ...
    /// Syntax `rsh` recognises but has not implemented.
    Unsupported {
